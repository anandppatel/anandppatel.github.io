...
\section{Preliminaries}
    ...
    \begin{remark}
        ...
        \begin{enumerate}
            \item We will use Lemma~\ref{lemma:extension} in a situation where $Y$ is a Grassmannian: every non-constant morphism out of a Grassmannian is necessarily finite (because the Grassmannian's Picard group is $\Z$), and so the critical hypothesis of Lemma~\ref{lemma:extension} will be satisfied.
            \item Lemma~\ref{lemma:extension} is true in arbitrary characteristic.
        \end{enumerate}
    \end{remark}
    ...
    The problem of determining the cycle class of graph closures of rational maps to projective spaces has, in a sense, a complete solution thanks to Aluffi in \cite{aluffi2015multidegrees}.  Making this explicit in particular cases is difficult, and in this section we will give a self-contained treatment in two very special situations.  These situations will correspond to the $\gcd(m,n) = 1$ and $\gcd(m,n) = 2$ categories mentioned in the introduction.
    ...
    \begin{proposition}
        ...
        \begin{enumerate}
            ...
            \item $X_{S}$ is the vanishing scheme of a section of the vector bundle $\L \boxtimes \QQ^{r-1}$ on $X \times \P \langle S \rangle^{*}$, where $\QQ^{r-1}$ is the rank $r-1$ universal quotient bundle over $\P \langle S \rangle^{*}$.
            ...
            \[
                [X_{T}] = \left[\frac{(1+\lambda)^{r+1}}{(1 + \lambda - \zeta)}\right]_{r} - \sum_{i=1}^{k} e_i [Y_{i} \times \P \langle T \rangle^{*}],
            \]
            where $\QQ^{r}$ is the rank $r$ universal quotient bundle over $\P \langle T \rangle^{*}$.
    ...
    \begin{proof}
        ...
        \[
            0 \to \O(-1) \to \O \otimes \langle S \rangle^{*} \to \QQ^{r-1} \to 0.
        \]
        Pulling these sequences back to $X \times \P \langle S \rangle^{*}$, and suppressing unsavory pull-back notation, we obtain on $X \times \P \langle S \rangle^{*}$ a homomorphism $s : \L^{-1} \to \QQ^{r-1}.$ We may equivalently view $s$ as a section of the vector bundle $\L \boxtimes \QQ^{r-1}$.  The description  of the embedding $X_S \hookrightarrow X \times \P \langle S \rangle^{*}$, in the proof of part (1) shows that $X_S$ is the vanishing scheme of $s$.
        ...
        \item This follows immediately from part (2), and the fact that the cycle class of $V(s)$ is the top Chern class of the corresponding vector bundle $\L \boxtimes \QQ^{r-1}$.  To compute this class, and to see why it is what has been indicated in the statement, simply tensor the tautological sequence above with $\L$ and use the Whitney sum formula.

        \item Each set $Y_{i} \times \P \langle T \rangle^{*}$ has codimension $r$ in $X \times \P \langle T \rangle^{*}$, and so the equality of sets $V(s)_{\red} = X_{T} \cup \left((Y_{T})_{\red} \times \P \langle T \rangle^{*}\right) = X_{T} \cup (\cup_{i} Y_{i} \times \P \langle T \rangle^{*})$ implies an equality of cycles of the form $[V(s)] = [X_{T}] + \sum_{i} e_i [Y_i \times \P \langle T \rangle^{*}].$  Since $V(s)$ attains its expected codimension $r$, the cycle class of $V(s)$ is the top Chern class of the corresponding rank $r$ vector bundle $\L \boxtimes \QQ^{r}$.  The assertion in part (4) follows.
   ...
   \begin{remark}
    ...
    In the proof of Proposition~\ref{lemma:indeterminacy}, we deduced the vanishing of the derivative of a morphism from its constancy. Therefore, Proposition~\ref{lemma:indeterminacy} critically uses the characteristic zero assumption on $\k$.
\end{remark}
...
\begin{proof}
    \label{proof:wronskiregular}
    This follows from the $m = n = d$ case of Proposition~\ref{lemma:indeterminacy}.
\end{proof}

\section{Introducing the variety of first transvectants}


Let us now fix positive integers $m, n, a, b, d$ such that $a m = b n = d$.  Consider the following commutative diagram involving the rational maps $\varphi_{a,b}: \P S_{m} \times \P S_{n} \dashrightarrow \Gr(2,S_{d})$ and $\epsilon: \P S_{m} \times \P S_{n} \dashrightarrow \P S_{m(a-1) + n(b-1)} \times \P S_{m+n-2}$:

Here $\varphi_{a,b}$ is the rational map $(f,g) \mapsto f^a \wedge g^b$, $\epsilon$ is the rational map $(f,g) \mapsto (f^{a-1}g^{b-1},\{f,g\})$, $\mult$ is the multiplication of forms, and $\upsilon$ is the map sending a pencil $p \wedge q$ to $\{p,q\}$, which is regular by Corollary~\ref{corollary:wronskiregular}.
...
\begin{definition}
    ...
    \[\beta: \transvectant_{m,n} \to \P S_{m} \times \P S_{n} \times \P S_{m+n-2}\] obtained by normalizing the closure of the graph of the first transvectant map $\P S_{m} \times \P S_{n} \dashrightarrow \P S_{m+n-2}$ given by $(f,g) \mapsto \{f,g\}$.
\end{definition}
...
\begin{proof}
    \label{proof:clebsch-resolved}
    \begin{enumerate}
        \item First note that $\epsilon$ extends to a morphism on $\transvectant_{m,n}$. Next we apply Lemma~\ref{lemma:extension} with $X = \transvectant_{m,n}$, $Y = \Gr(2,S_{d})$, $Z = \P S_{2d-2}$, $\alpha = \varphi_{a,b}$, and $\beta = \upsilon$, along with the commutativity of the diagram above to reach the desired conclusion.
...
From Proposition~\ref{proposition:clebsch-resolved} we obtain a picture where $\beta_{12}$ denotes the composite $\transvectant_{m,n} \xrightarrow{\beta} \P S_{m} \times \P S_{n} \times \P S_{m+n-2} \xrightarrow{p_{12}} \P S_{m} \times \P S_{n}$, and $\widetilde{\epsilon}$ is the extension of $\epsilon$ to $\transvectant_{m,n}$.  Observe that $\beta_{12}$ is a birational morphism.  This shows how $\transvectant_{m,n}$ is crucial to the problem of counting sums of two powers, and so we will study some of its enumerative geometry next.
...
\]
a general element of $\P S_{e}$.  If
\begin{align*}
...
\begin{proof}
    \label{proof:transvectantcoefficients}
...
\begin{proof}
    ...
    As $\widetilde{\varphi_{a,b}}(t)$ is $T$-fixed, it follows that $\widetilde{\varphi_{a,b}}(t) = (x^{i}y^{m-i})^{a} \wedge x^{u}y^{v}$ for some $u,v$. To extract more information, observe that the point $\upsilon(\widetilde{\varphi_{a,b}}(t))$ can be read directly from the point $t$ by going the other way around the diagram. The result is $\upsilon(\widetilde{\varphi_{a,b}}(t)) = (x^{i}y^{m-i})^{a-1}(x^{j}y^{n-j})^{b-1}(x^{k}y^{m+n-2-k}) \in \P S_{2d-2}.$  On the other hand, by directly computing the transvectant $\{(x^{i}y^{m-i})^{a} , x^{u}y^{v} \}$ we find that
    \begin{align*}
        u &= -(i-1) + (b-1)j + k\\
...
Though Proposition~\ref{proposition:weightsofS} is quite elementary, the next few corollaries show how it points the way to the general solution to the problem of counting sums of powers.
...
\begin{theorem}
    ...
    \begin{proof}
        \label{proof:czvclass}
        Recall that, by its very construction, $\beta(\transvectant_{m,n})$ is the closure of the graph of the first transvectant map.  We will refer to Proposition~\ref{proposition:resolution} in the special case where $X = \P S_{m} \times \P S_{n}$, $Y = \beta(\transvectant_{m,n}) \subset X \times \P S_{m+n-2}$, and $\L = p_1^{*}\O(1) \otimes p_2^{*}\O(1)$ on $X$, noting that the first transvectant map is given by bi-degree $(1,1)$ polynomials.

Observe:
\begin{enumerate}
    \item[(A)] When $\gcd(m,n) = 1$, the scheme $Y \subset \P S_{m} \times \P S_{n}$ defined by $\{f,g\} = 0$ is the common vanishing scheme of $m+n-1$ sections of $\L$ {\sl and has codimension $m+n-1$}, by Proposition~\ref{lemma:indeterminacy}. This falls within the scope of the ``S'' case of Proposition~\ref{proposition:resolution}.
    \item[(B)] When $\gcd(m,n) = 2$, the scheme $Y \subset \P S_{m} \times \P S_{n}$ defined by $\{f,g\} = 0$ is the common vanishing scheme of $m+n-1$ sections of $\L$ {\sl and has codimension $m+n-2$}, by Proposition~\ref{lemma:indeterminacy}. This falls within the scope of the ``T'' case of Proposition~\ref{proposition:resolution}.
\end{enumerate}

We now take both statements in the theorem in turn.

    \begin{enumerate}
        \item This is immediate from part (3) of Proposition~\ref{proposition:resolution} in light of observation (A) above.
        \item Part (4) of Proposition~\ref{proposition:resolution} applies thanks to observation (B) above.  We conclude that there exists a positive integer $e$ such that
        \[
        ...
        Part (5) of Proposition~\ref{proposition:resolution} tells us how to compute $e$.  Let
        \begin{align*}
...
\begin{remark}
    \label{remark:ZariskiPairs} The sextic curves mentioned in Question~\ref{question:higherforms} famously form one of the two irreducible components of the variety parametrizing sextics with six cusps.  These sextics are branch curves of linear projections of cubic surfaces.  The other component is more mysterious, and the author does not know any good presentation for their equations.  This is the starting point of the beautiful topic of Zariski pairs.
\end{remark}

\begin{remark}
    \label{remark:notdaunting} Although Question~\ref{question:higherforms} may seem daunting, we note that the base scheme of the rational map $\P^{5} \times \P^{9} \dashrightarrow \G(1,27)$ sending a general pair $(Q,C)$ consisting of a ternary quadric and cubic to the pencil $Q^{3} \wedge C^{2}$ is ill-defined precisely on the locus of pairs $(L^{2},L^{3})$ where $L$ is a linear form.  This locus consists of a single $\GL(3)$-orbit, and so there aren't any embedded primes in the indeterminacy {\sl scheme} of this rational map.  This provides considerable confidence in attempting to find a workable resolution.
\end{remark}

\begin{question}
    \label{question:fundamentalclassofacovariant}  The first transvectant $(f,d) \mapsto \{f,g\}$ is one of the most important examples of an $\SL(2)$-covariant of binary forms.  But there are plenty more covariants, including higher transvectants.  Each of these has a fundamental class, by which we mean the fundamental class of the graph closure as in Theorem~\ref{theorem:czvclass}.  What are these classes?
\end{question}
...
\begin{thebibliography}{Vak01}

\bibitem[Alu15]{aluffi2015multidegrees}
Paolo Aluffi.
\textit{Multidegrees of monomial rational maps.}
Publ. Res. Inst. Math. Sci., 51(4):635--654, 2015.

\bibitem[Cle70]{clebsch1870theorie}
A.~Clebsch.
\textit{Zur Theorie der bin\"aren Formen sechster Ordnung und zur Dreitheilung der hyperelliptischen Functionen.}
Math. Ann., 2(2):193--197, 1870.

\bibitem[EG98]{edidin1998localization}
Dan Edidin and William Graham.
\textit{Localization in equivariant intersection theory and the Bott residue formula.}
Amer. J. Math., 120(3):619--636, 1998.

\bibitem[Vak01]{vakil2001twelve}
Ravi Vakil.
\textit{Twelve points on the projective line, branched covers, and rational elliptic fibrations.}
Math. Ann., 320(1):33--54, 2001.

\bibitem[Zar28]{zariski1928hyperelliptic}
Oscar Zariski.
\textit{On hyperelliptic $\theta$-functions with rational characteristics.}
Amer. J. Math., 50(3):315--344, 1928.
