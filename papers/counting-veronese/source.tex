...
If $\left([T], p_{1}, \dots, p_{13} \right)$ is an element of $\mathcal{Y}$, we say $T$ \text{is a {\bf singular triad} for $p_{1}, \dots, p_{13}$.}
...
\begin{proof}
  ...
  $(\P^{2})^{12}$, and hence the set of $13$ \text{general} points
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        If $T_{n}$ has length $3$ and is curvilinear, then in suitable \text{analytic} local coordinates $(u,v)$ around $n$, the ideal $\I_{T_{n}}$ can be taken to be $(u,v^{3})$, and so $\I_{\s(T,T^{*})} = (u^{2},uv^{3},v^{6})$.  Once again, an analytic local equation for $Q$ cannot  be contained in $\I_{\s(T,T^{*})}$ because it would have a non-degenerate quadratic term. 
...
\begin{enumerate}
  \item \text{Assuming $\deg (C) = 4$},  the moving part of $\Lambda$ is a pencil of
  ...
  \item \text{Assuming $\deg (C) = 3$}, the moving part $\Lambda'$ of $\Lambda$ is a pencil of
  ...
  \item \text{Assuming $\deg (C) = 2$}, then once again $C$ must be reduced as is seen by
...
Now we consider the finite, length 4 subscheme $Z := \widetilde{p} \cup \widetilde{\mathcal{T}}_{0} \subset E$.  On $E$, the term {\sl line} refers to any \text{irreducible} curve in the linear series $|D+F|$ on $E$.  In particular, lines do not intersect the directrix $D$. 
...
\begin{remark}
    In this direction, a dimension count suggests that for a fixed set of $14$ general points $a_1, \dots, a_{14}$ in $\P^{3}$, there should exist finitely many \text{quadro-quartic} Cremona transformations $\gamma: \P^{3} \dashrightarrow \P^{3}$ such that the composition of $\gamma$ with a $2$-uple Veronese embedding sends the tuple $(a_{1}, \dots, a_{14})$ to an associated tuple $(b_{1}, \dots, b_{14}) \in (\P^{9})^{14}$.  This may hint at an approach, admittedly ambitious, to finding the number $\nu_{2,3}$ of $2$-Veronese $3$-folds through $14$ general points.
\end{remark}   
